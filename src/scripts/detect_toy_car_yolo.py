...
CONF_THRESH = 0.30
TARGET_CLASSES = ["car", "truck", "bus"]  # on cible 'car' — peut inclure d'autres classes

def main():
    cap = cv2.VideoCapture(0)  # 0 = caméra principale
    if not cap.isOpened():
        print("Impossible d'ouvrir la caméra. Vérifie les permissions macOS (Préférences Système → Sécurité).")
        return
    ...
    while True:
        ...
        fps_time = time.time()
        cv2.putText(frame, f"FPS: {fps:.1f}  Detected: {len(boxes)}", (10, 20),
